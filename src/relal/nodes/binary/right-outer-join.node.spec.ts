...
import { RightOuterJoinNode } from './right-outer-join.node';

nodeTestCase('RightOuterJoinNode', visit => {
  it('should return the given value on visit - null value', async () => {
    const node = new RightOuterJoinNode(
      new SqlLiteralNode('TABLE'),
      null,
    );
    const collector = visit(node);
    expect(collector.value).toBe('RIGHT OUTER JOIN TABLE');
  ...
  it('should return the given value on visit - null value', async () => {
    ...
    const collector = visit(node);
    expect(collector.value).toBe('RIGHT OUTER JOIN TABLE ON RIGHT SIDE');
    expect(collector.bindIndex).toBe(0);
    expect(collector.boundValues).toStrictEqual([]);
  });

  it('should be accessible using register', async () => {
    expect(node('right-outer-join')).toStrictEqual(RightOuterJoinNode);
  });
});
