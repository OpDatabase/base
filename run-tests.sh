...
export TEST_PG_DB_NAME=pg-test-opdb
...
export TEST_MYSQL_DB_NAME=mysql-test-opdb
...
(docker kill $TEST_MYSQL_DB_NAME || true) > /dev/null 2> /dev/null
docker run \
...
docker kill $TEST_MYSQL_DB_NAME > /dev/null
